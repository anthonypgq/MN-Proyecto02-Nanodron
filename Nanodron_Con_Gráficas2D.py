...
def run_simulation():
    try:
        ...
        messagebox.showerror(
            "Entrada inválida", "Por favor, ingrese valores numéricos válidos para α, β, y γ.")
        return
    ...
    # Límites de los ejes
    x_min, x_max = min(solution_A[:, 0].min(), solution_B[:, 0].min()), max(
        solution_A[:, 0].max(), solution_B[:, 0].max())
    y_min, y_max = min(solution_A[:, 1].min(), solution_B[:, 1].min()), max(
        solution_A[:, 1].max(), solution_B[:, 1].max())
    z_min, z_max = min(solution_A[:, 2].min(), solution_B[:, 2].min()), max(
    ...
    ax_3d.set_title('Trayectoria del Nanodron', fontsize=16,
                    fontweight='bold', color='navy')

    # Inicialización de las líneas para las trayectorias
    line_A_3d, = ax_3d.plot([], [], [], label='Trayectoria A',
                            color='orange', linewidth=2)
    line_B_3d, = ax_3d.plot([], [], [], label='Trayectoria B',
    ...
    ax_3d.scatter(initial_conditions_A[0], initial_conditions_A[1],
                  initial_conditions_A[2], color='red', s=100, edgecolor='black', label='Inicio A')
    ax_3d.scatter(initial_conditions_B[0], initial_conditions_B[1],
                  initial_conditions_B[2], color='blue', s=100, edgecolor='black', label='Inicio B')
    ...
    params_text = ax_3d.text2D(0.05, 0.85, '', transform=ax_3d.transAxes,
                               fontsize=12, color='black', backgroundcolor='white')

    # Indicador del tiempo transcurrido
    time_template = 'Tiempo = %.1f s'
    time_text = ax_3d.text2D(0.05, 0.80, '', transform=ax_3d.transAxes,
                             fontsize=12, color='black', backgroundcolor='white')

    # Añadir puntos finales
    final_point_A, = ax_3d.plot(
        [], [], [], 'o', color='yellow', markersize=8, label='Final A')
    final_point_B, = ax_3d.plot(
        [], [], [], 'o', color='green', markersize=8, label='Final B')
    ...
    def init_3d():
        ...
        return line_A_3d, line_B_3d, projection_A_x, projection_A_y, projection_A_z, projection_B_x, projection_B_y, projection_B_z, final_point_A, final_point_B, time_text, params_text
    ...
    def update_3d(num):
        ...
        projection_A_x.set_data([solution_A[num, 0], solution_A[num, 0]], [
                                solution_A[num, 1], solution_A[num, 1]])
        projection_A_x.set_3d_properties([z_min, solution_A[num, 2]])
        projection_A_y.set_data([solution_A[num, 0], solution_A[num, 0]], [
                                y_min, solution_A[num, 1]])
        projection_A_y.set_3d_properties(
            [solution_A[num, 2], solution_A[num, 2]])
        projection_A_z.set_data([x_min, solution_A[num, 0]], [
                                solution_A[num, 1], solution_A[num, 1]])
        projection_A_z.set_3d_properties(
            [solution_A[num, 2], solution_A[num, 2]])

        # Proyecciones desde el nanodron B hacia los ejes
        projection_B_x.set_data([solution_B[num, 0], solution_B[num, 0]], [
                                solution_B[num, 1], solution_B[num, 1]])
        projection_B_x.set_3d_properties([z_min, solution_B[num, 2]])
        projection_B_y.set_data([solution_B[num, 0], solution_B[num, 0]], [
                                y_min, solution_B[num, 1]])
        projection_B_y.set_3d_properties(
            [solution_B[num, 2], solution_B[num, 2]])
        projection_B_z.set_data([x_min, solution_B[num, 0]], [
                                solution_B[num, 1], solution_B[num, 1]])
        projection_B_z.set_3d_properties(
            [solution_B[num, 2], solution_B[num, 2]])

        final_point_A.set_data(
            solution_A[num-1:num, 0], solution_A[num-1:num, 1])
        final_point_A.set_3d_properties(solution_A[num-1:num, 2])
        final_point_B.set_data(
            solution_B[num-1:num, 0], solution_B[num-1:num, 1])
    ...
    # Animación
    ani_3d = FuncAnimation(fig_3d, update_3d, frames=len(
        t), init_func=init_3d, blit=True, interval=20, repeat=False)
    ...
    line_A_xy, = axes_2d[0].plot(
        [], [], label='Trayectoria A', color='orange', linewidth=2)
    line_B_xy, = axes_2d[0].plot(
        [], [], label='Trayectoria B', color='purple', linewidth=2)
    line_A_xz, = axes_2d[1].plot(
        [], [], label='Trayectoria A', color='orange', linewidth=2)
    line_B_xz, = axes_2d[1].plot(
        [], [], label='Trayectoria B', color='purple', linewidth=2)
    line_A_yz, = axes_2d[2].plot(
        [], [], label='Trayectoria A', color='orange', linewidth=2)
    line_B_yz, = axes_2d[2].plot(
        [], [], label='Trayectoria B', color='purple', linewidth=2)
    ...
    def init_2d():
        ...
        line_A_yz.set_data([], [])
        line_B_yz.set_data([], [])
        return line_A_xy, line_B_xy, line_A_xz, line_B_xz, line_A_yz, line_B_yz
    ...
    ani_2d = FuncAnimation(fig_2d, update_2d, frames=len(
        t), init_func=init_2d, blit=True, interval=20, repeat=False)
...
run_button = tk.Button(root, text="Ejecutar Simulación",
                       font=("Arial", 14), command=run_simulation)
run_button.grid(row=4, columnspan=2, pady=20)

# Etiquetas de los integrantes del grupo
group_label = tk.Label(root, text="Integrantes del Grupo:",
                       font=("Arial", 12, "bold"))
group_label.grid(row=5, columnspan=2)

members_label = tk.Label(
    root, text="Anthony Goyes, David Arciniegas, Ozzy Loachamin, Angel Falcon", font=("Arial", 12))
